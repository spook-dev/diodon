...
public class substitution_solver {
	...
	public void populate_patternTable(){
		File pattern_dir = new File("../pattern_data/patterns");
		File[] pattern_files = pattern_dir.listFiles();
		File c_file;
		String pattern_template;
		for(int i = 0; i < pattern_files.length; i++){
			c_file = pattern_files[i];
			pattern_template = c_file.getName();
			if(pattern_template.indexOf(".") > 0)
				pattern_template = pattern_template.substring(0, pattern_template.lastIndexOf("."));
	...
	public ArrayList<key> crack(String ct){
		...
		for(int i = 0; i < words.length; i++){
			...
			pattern_matches = patternTable.get(generate_pattern(words[i])); //this is full of words, it needs to be full of keys
		...
		Collections.sort(words_tree, (l1, l2) -> (l1.size() - l2.size())/(Math.abs(l1.size() - l2.size()) == 0 ? 1 : Math.abs(l1.size() - l2.size()))); //sort shortest keys first
	...
	public ArrayList<key> AdvancedCrack(String ct, boolean doPrint) {
		...
		for(int l = words.size(); l > 0; l--){
			...
			for(ArrayList<String> i : combos){
				if(doPrint){
					System.out.format("\r%s - %s/%s", string_utils.percentagebar(0, possibilities, c), c, possibilities); //tk make this function
					//~ Thread.sleep(100);
					//~ sys.stdout.flush()
				}
				String current_ct = "";
